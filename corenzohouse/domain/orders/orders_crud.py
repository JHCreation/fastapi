...
async def order_get_list( db: Session, params: dict ):
    query = select(Orders)

    if params.get('key') is not None:
        query = query.where(Orders.key.ilike(f"%{params['key']}%"))
    if params.get('min_date') is not None:
        query = query.where(Orders.create_date >= params['min_date'])
    if params.get('max_date') is not None:
        query = query.where(Orders.create_date <= params['max_date'])
    if params.get('oid') is not None:
        if not isinstance(params['oid'], list):  # 리스트 타입인지 확인
            query = query.where(Orders.order_id == params['oid'])
        else:
            query = query.where(Orders.order_id.in_(params['oid']))
