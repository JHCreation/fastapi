...
from ....model.eightpointone.eightpointone_model import Partners
from ....config import logger
from .partners_schema import PartnersCreate, PartnersBase, PartnersUpdate, PartnersDeletes
...
async def create(
    ...
    return await comm_crud.async_create(Partners, db, item, res_id='id', update=update)

@router.put("/update/{id}")
async def update(
    id: str,
    db: AsyncSession = Depends(get_async_db),
    params: PartnersBase = Body(...),
    api_key: str = Security(api_bearer_token)
):
    update= {
        'modify_date' : datetime.now(),
    }
    return await comm_crud.async_update(Partners, db, params, filter_key='id', filter_value=id, res_id='id', update=update)
...
async def delete(
    ...
    # print(item)
    # return
    return await comm_crud.async_delete(Partners, db, filter_key='id', filter_value=id, res_id='id')
 

@router.delete("/deletes")
async def works_deletes(
    param: PartnersDeletes,
    # ids: List[int]= Query(...),
    db: AsyncSession = Depends(get_async_db),
    api_key: str = Security(api_bearer_token)
):
    ...
    return await comm_crud.async_deletes(Partners, db, filter_key='key', filter_value=param.ids)